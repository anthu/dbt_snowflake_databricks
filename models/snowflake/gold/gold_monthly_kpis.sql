{{
  config(
    materialized='table',
    tags=['gold', 'business', 'kpi', 'monthly']
  )
}}
...
with monthly_base as (
    select
        ...
        extract(year from l.order_date) as report_year,
        extract(month from l.order_date) as report_month_num,
        l.*
...
current_month as (
    select
        ...
        -- customer metrics
        sum(final_price) / nullif(count(distinct customer_key), 0) as revenue_per_customer,
        sum(final_price) / nullif(count(distinct order_key), 0) as avg_order_value,
        count(*) / nullif(count(distinct order_key), 0) as avg_lines_per_order,
...
select
    ...
    cm.domestic_revenue / nullif(cm.net_revenue, 0) as domestic_revenue_pct,
    cm.international_revenue / nullif(cm.net_revenue, 0) as international_revenue_pct,
    
    -- priority metrics
    cm.high_priority_revenue,
    cm.medium_priority_revenue,
    cm.low_priority_revenue,
    cm.high_priority_revenue / nullif(cm.net_revenue, 0) as high_priority_revenue_pct,
