{{
  config(
    materialized='table',
    tags=['gold', 'business', 'regional', 'analysis']
  )
}}
...
select
    ...
    -- calculated metrics
    sum(l.final_price) / nullif(count(distinct l.customer_key), 0) as revenue_per_customer,
    sum(l.final_price) / nullif(count(distinct l.order_key), 0) as revenue_per_order,
    sum(l.quantity) / nullif(count(distinct l.customer_key), 0) as quantity_per_customer,
