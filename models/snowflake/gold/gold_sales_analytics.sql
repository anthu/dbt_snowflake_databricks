{{
  config(
    materialized='table',
    tags=['gold', 'business', 'sales', 'analytics']
  )
}}
...
select
    ...
    sum(l.final_price) / nullif(sum(l.quantity), 0) as revenue_per_unit,
    sum(l.discount_amount) / nullif(sum(l.extended_price), 0) as discount_rate_actual,
