{{
  config(
    materialized='table',
    tags=['silver', 'cleaned', 'dimension', 'part']
  )
}}
...
select
    ...
    split_part(upper(trim(p_type)), ' ', 1) as type_category,
    split_part(upper(trim(p_type)), ' ', 2) as type_modifier,
    split_part(upper(trim(p_type)), ' ', 3) as type_material,
